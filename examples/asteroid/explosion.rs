    ...
    for event in event_reader.iter() {
        let (texture_name, sound_name, start_scale, end_scale, duration) = match event.kind {
            ExplosionKind::ShipDead => (
                "asteroid/explosion01.png",
                "asteroid/Explosion_ship.mp3",
                0.1 / 15.0,
                0.5 / 15.0,
                1.5,
            ),
            ExplosionKind::ShipContact => {
                ("asteroid/flash00.png", "asteroid/Explosion.mp3", 0.05 / 15.0, 0.1 / 15.0, 0.5)
            }
            ExplosionKind::LaserOnAsteroid => {
                ("asteroid/flash00.png", "asteroid/Explosion.mp3", 0.1 / 15.0, 0.15 / 15.0, 0.5)
            }
        };
